...
contract ResultManager is AccessControlEnumerable {
    ...
    /// @notice mapping for Collection name -> Value Info
    ...
    function updateResult(
        ...
        if (
            ...
            _collectionResults[result.name].lastUpdatedTimestamp
        ) {
            if (!_checkSignature(merkleRoot, signature))
                revert InvalidSignature();

            if (!_checkMerkle(result, merkleRoot, proof))
    ...
    function _checkSignature(
        ...
        bytes32 messageHash = keccak256(abi.encodePacked(merkleRoot));
    ...
    function _checkMerkle(
        ...
        bytes32 leaf = keccak256(bytes.concat(keccak256(abi.encode(result))));
